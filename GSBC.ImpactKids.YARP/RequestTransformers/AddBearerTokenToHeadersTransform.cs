...
internal sealed partial class AddBearerTokenToHeadersTransform(ILogger<AddBearerTokenToHeadersTransform> logger) : RequestTransform
{
    public override async ValueTask ApplyAsync(RequestTransformContext context)
    {
        ...
        {
            LogCouldNotGetAccessTokenGetUserAccessTokenErrorForRequestPathRequestPathError(logger, accessToken.FailedResult.Error, context.HttpContext.Request.Path.Value, accessToken.FailedResult.ErrorDescription);
            return;
        }
    ...
    static partial void LogCouldNotGetAccessTokenGetUserAccessTokenErrorForRequestPathRequestPathError(ILogger<AddBearerTokenToHeadersTransform> logger, string? getUserAccessTokenError, string? requestPath, string? error);

    [LoggerMessage(LogLevel.Information, "Adding bearer token to request headers for request path: {RequestPath}")]
    static partial void LogAddingBearerTokenToRequestHeadersForRequestPathRequestPath(ILogger<AddBearerTokenToHeadersTransform> logger, string? requestPath);
}